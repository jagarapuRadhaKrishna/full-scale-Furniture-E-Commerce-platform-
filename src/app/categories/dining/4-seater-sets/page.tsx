...
const fourSeaterSets = categoryProducts['Dining']?.filter(product => 
  product.name.toLowerCase().includes('4-seater') || 
  product.name.toLowerCase().includes('four seater') ||
  product.subcategory?.toLowerCase().includes('4-seater')
) || []

export default function FourSeaterSetsPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white">
      <ModuleHeader 
        ...
        description="Perfect dining sets for families with elegant designs and quality craftsmanship"
        breadcrumbs={[
          { label: 'Home', href: '/' },
          { label: 'Categories', href: '/categories' },
          { label: 'Dining', href: '/categories/dining' },
          { label: '4-Seater Sets', href: '/categories/dining/4-seater-sets' }
        ]}
      />

      <div className="container mx-auto px-4 py-8">
        {fourSeaterSets.length === 0 ? (
          <div className="text-center py-16">
            <h3 className="text-xl text-gray-600 mb-4">4-Seater Dining Sets Collection Coming Soon</h3>
            <p className="text-gray-500 mb-6">We're curating the perfect collection of 4-seater dining sets for you.</p>
            <Link 
              href="/categories/dining" 
              className="bg-purple-600 text-white px-6 py-3 rounded-lg hover:bg-purple-700 transition-colors"
            >
              Explore All Dining Furniture
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {fourSeaterSets.map((product) => (
